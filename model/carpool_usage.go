...
func buildCarpoolUsageSummary(group string, period string, start time.Time, end time.Time, rangeOnly bool, session *CarpoolSessionSummary) (*CarpoolUsageSummarySnapshot, error) {
	...
	carnivalPeriodUsers, err := queryCarpoolCarnivalUserUsage(group, startTs, endTs, 0)
	if err != nil {
		return nil, err
	}
	...
	carnivalPeriodTokens, err := queryCarpoolCarnivalTokenUsage(group, startTs, endTs, 0)
	if err != nil {
		return nil, err
	}
...
func queryCarpoolQuotaDataFallbackUserUsage(group string, start time.Time, end time.Time) ([]carpoolUsageRow, error) {
	...
	query := DB.Table("quota_data").
		Where("user_id IN (?)", DB.Model(&Token{}).Select("user_id").Where(commonGroupCol+" = ?", group))
	if startTs > 0 {
...
func carpoolTimestampBoundsBeforeFirstSnapshot(group string, start time.Time, end time.Time) (int64, int64, bool) {
	...
	minDate, err := getFirstCarpoolSnapshotDate(group)
	if err != nil {
		return 0, 0, false
	}
	if minDate != "" {
		firstSnapshotDay, err := time.ParseInLocation("2006-01-02", minDate, time.Local)
		if err != nil {
			return 0, 0, false
		}
		beforeSnapshotEnd := firstSnapshotDay.Unix() - 1
...
func getFirstCarpoolSnapshotDate(group string) (string, error) {
