...
export const GroupRatioForm = memo(function GroupRatioForm({
  ...
  const { userGroupOptions, pricingGroupOptions } = useMemo(
    () => buildSpecialUsableOptions(groupValues),
    [
      groupValues.GroupRatio,
      groupValues.TopupGroupRatio,
      groupValues.UserUsableGroups,
      groupValues.GroupGroupRatio,
      groupValues.AutoGroups,
        ...
        {editMode === 'visual' ? (
          <div className='space-y-6'>
            <GroupRatioVisualEditor
              groupRatio={groupValues.GroupRatio}
              topupGroupRatio={groupValues.TopupGroupRatio}
              userUsableGroups={groupValues.UserUsableGroups}
              groupGroupRatio={groupValues.GroupGroupRatio}
              autoGroups={groupValues.AutoGroups}
            ...
            <GroupSpecialUsableRulesEditor
              value={groupValues.GroupSpecialUsableGroup}
...
vip          0.5     ${t('No')}                ${t('Assigned by administrator only')}`}
    ...
    "premium": 0.3
            ...
            <AccordionItem value='usable'>
              ...
              <AccordionContent className='space-y-3'>
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'Special usable group rules can add, remove, only allow, or append selectable token groups for a specific user group.'
                  )}
                </p>
                <GuideCodeBlock>{`{
  "vip": {
  ...
  "jhl": {
    ...
    "=:jhl": "jhl分组"
                ...
                <p className='text-muted-foreground text-sm leading-6'>
                  {t(
                    'Use +: to add a group, -: to remove a default selectable group, =: to allow only listed groups, or no prefix to append a group.'
                  )}
                </p>
              </AccordionContent>
            </AccordionItem>
