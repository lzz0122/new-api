  ...
  const renderVisualMode = () => (
    <Form key='form-visual' values={inputs} style={{ marginBottom: 15 }}>
      ...
      <Form.Section text={t('分组特殊可用分组')}>
        ...
        >
          {t(
            '为特定用户分组配置可用分组规则。「仅允许」会覆盖默认可选列表，「添加」新增可用分组，「移除」移除默认可用分组，「追加」直接追加分组',
          )}
        </Text>
        ...
        <Row gutter={16}>
          <Col xs={24} sm={16}>
            <Form.TextArea
              ...
              placeholder={t('为一个 JSON 文本')}
              extraText={t(
                '键为用户分组名称，值为操作映射对象。内层键以"=:"开头表示仅允许指定分组并覆盖默认可选列表，以"+:"开头表示添加指定分组，以"-:"开头表示移除指定分组，不带前缀的键直接添加该分组。例如：{"jhl": {"=:拼车": "拼车分组", "=:jhl": "jhl分组"}}，表示 jhl 分组用户只能使用拼车和 jhl 分组',
              )}
        ...
        <Tabs.TabPane tab={t('可用分组')} itemKey='usable'>
          <div style={{ paddingTop: 20 }}>
            ...
            <GuideSection title={t('查看示例')}>
              ...
              >
                {t(
                  '场景：站点有 standard 和 premium 两个用户可选分组。希望 vip 用户额外看到 exclusive 分组，同时不再看到 standard 分组；也可以为 jhl 用户配置只显示拼车和 jhl',
                )}
              </Paragraph>
              <Paragraph
                size='small'
            ...
            <GuideSection title={t('JSON 格式参考')}>
              ...
              >
                {t(
                  '键的前缀 =: 表示仅允许，+: 表示添加，-: 表示移除，无前缀表示追加。值为分组描述（移除时填 "remove"）。',
                )}
              </Paragraph>
            </GuideSection>
          </div>
